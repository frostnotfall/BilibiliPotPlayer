...
void AppendBangumiQualityList(const string epid, const string path, string& url, array<dictionary>& QualityList) {
	status = 5;
	...
	if (!Root["data"]["result"].isObject()) return;
	...
	if (!Root["data"]["result"]["video_info"].isObject()) return;
	...
	if (reader.parse(res, root) && root.isObject()) {
		if (root["code"].asInt() == 0) {
			...
			if (data["dash"].isObject()) {
				...
				HostMessageBox('告知我一下, /x/player/wbi/playurl has data["durl"]', "告知我一下", 0, 0);
			}
		} else {
...
JsonValue SortVideos(JsonValue& videos) {
	...
	for (uint i = 0; i < videos.size(); i++) {
		...
		for (uint j = 0; j < videos.size(); j++) {
			...
			int order = int(VideoIdOrder.length());
...
array<dictionary> BangumiEpisodes(string id, string type) {
	...
	for (int j = 0; j < episodes.size(); j++) {
		...
		dictionary video;
		video["title"] = episode["share_copy"].asString() + "\n" + episode["show_title"].asString();
	...
	for (int i = 0; i < sections.size(); i++) {
		...
		for (int j = 0; j < episodes.size(); j++) {
			...
			for (int k = 0; k < pages.size(); k++) {
				...
				video["referer"] = "https://www.bilibili.com/video/" + episode["bvid"].asString();
				videos.insertLast(video);
			}
		}
	}
...
string Bangumi(const string&in path, dictionary& MetaData, array<dictionary>& QualityList) {
	...
	for (int i = 0; i < section["pages"].size(); i++) {
		...
		for (int j = 0; j < episodes.size(); j++) {
			...
			if (@MetaData !is null) {
				string title = episode["share_copy"].asString() + "\n" + episode["show_title"].asString();
	...
	AppendBangumiQualityList(epid, path, url, QualityList);
...
string Video(string id, const string&in path, dictionary& MetaData, array<dictionary>& QualityList) {
	...
	if (view["pages"].isArray() && view["pages"].size() > 0) {
		...
		if (page["cid"].isString() && !page["cid"].asString().empty()) {
			cid = page["cid"].asString();
		}

		if (view["pages"].size() > 1) {
			title = view["title"].asString() + " > " + page["part"].asString();
		}
	}
...
array<dictionary> liveCategory(string path) {
	JsonReader Reader;
	JsonValue Root;
	...
	int parentAreaId = parseInt(HostRegExpParse(path, "parentAreaId=([0-9]+)"));
	int areaId = parseInt(HostRegExpParse(path, "areaId=([0-9]+)"));

	if (path.find("lol") >= 0) {
		return liveCategory(86, 2, w_webid);
	}
	if (path.find("pubg") >= 0) {
		return liveCategory(80, 2, w_webid);
	}
	if (path.find("cf") >= 0) {
		return liveCategory(88, 2, w_webid);
	}
	if (path.find("hpjy") >= 0) {
		return liveCategory(256, 3, w_webid);
	}

	return liveCategory(areaId, parentAreaId, w_webid);
}

array<dictionary> liveCategory(int cateid, int parentAreaId, string w_webid) {
	array<dictionary> videos;
	JsonReader Reader;
	JsonValue Root;
	int page = 1;
